...
describe("pick", () => {
    test("pick root or dynamic", () => {
        ...
        expect(pick(routes1, "/").route.value).toBe("root");
    });
    ...
    test("dynamic segments + splat return value", () => {
        const routes2 = [{ path: "/users/:userId/files/*" }];
        expect(pick(routes2, "/users/ryan/files/some/deep/path")).toMatchSnapshot();
    });

    test("query strings", () => {
        const routes3 = [{ path: "/users/:userId" }];
        expect(pick(routes3, "/users/ryan?cool=stuff")).toMatchSnapshot();
    });
});
